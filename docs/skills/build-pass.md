...
**Pass 2:** Correct data shown; backend is source of truth; loading/empty states safe. If using an SDK hook for requests, verify how failures surface (promise reject vs error/status state). Do not rely on try/catch unless confirmed. Test at least one non-2xx failure path.  
**Pass 3:** Logic correct and readable; results match expectations; edge cases safe.  
**Pass 4:** Feature cohesive; no unnecessary complexity; matches app patterns. If using an SDK hook for requests, verify how failures surface (promise reject vs error/status state). Do not rely on try/catch unless confirmed. Test at least one non-2xx failure path.
...
When Pass 4 is done, **Next** should state that implementation passes are complete (or equivalent).
